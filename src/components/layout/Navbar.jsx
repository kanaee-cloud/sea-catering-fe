    ...
    <div className="relative p-5">
      <header className="glassmorphism relative shadow-md p-4 top-0 z-50 rounded-full">
        <div className="container mx-auto flex justify-between items-center">
          {/* Logo */}
          <h1 className="text-xl font-bold ">SEA</h1>
          ...
          <div className="hidden md:block">
            <button className="text-sm font-medium text-gray-900 hover:text-black transition-colors duration-200 px-4 py-2 border border-gray-300 rounded-full hover:border-black">
              Login
            </button>
          </div>
        ...
        <div
          className={`md:hidden absolute top-full left-0 right-0 mt-2 mx-5 bg-white rounded-2xl shadow-lg border transition-all duration-300 ease-out ${
            isMobileMenuOpen
          ...
          <nav className="py-4">
            {navItems.map((item) => (
              <a
                key={item.id}
                href={item.href}
                className={`block px-6 py-3 text-sm font-medium transition-all duration-200 relative ${
                  activeRoute === item.id
